...
static int formatComponents(int fmt)
{
  ...
  LOG_(ImageLog,plog::debug) << "formatComponents \t" << fmt << "\n";
    switch (fmt)
    {
    case GL_RGBA:
    case GL_BGRA_EXT:
...
static int calcMipLevelSize(int fmt, int w, int h, int mip)
{
  ...
  LOG_(ImageLog,plog::debug) << "calcMipLevelSize \t" << fmt << "\t" << w << "\t" << h << "\t" << mip << "\n";
    w = max(w >> mip, 1);
    h = max(h >> mip, 1);
    ...
    pixels(NULL)
{
  ...
  LOG_(ImageLog,plog::debug) << "Image CTOR \t" << fmt << "\t" << w << "\t" << h << "\t" << mips << "\n";
...
int Image::getWidth() const
{
  ...
  LOG_(ImageLog,plog::debug) << "Image::getWidth \t" << width << "\n";
    return width;
}
...
int Image::getHeight() const
{
  ...
  LOG_(ImageLog,plog::debug) << "Image::getHeight \t" << height << "\n";
    return height;
}
...
int Image::getPitch() const
{
	...
	LOG_(ImageLog,plog::debug) << "Image::getPitch \t" << pitch << "\n";
    return pitch;
}
...
int Image::getMipLevelCount() const
{
	...
	LOG_(ImageLog,plog::debug) << "Image::getMipLevelCount \t" << mipLevels << "\n";
    return mipLevels;
}
...
int Image::getSize() const
{
	...
	LOG_(ImageLog,plog::debug) << "Image::getSize \t" << size << "\n";
    return size;
}
...
int Image::getFormat() const
{
	...
	LOG_(ImageLog,plog::debug) << "Image::getFormat \t" << format << "\n";
    return format;
}
...
int Image::getComponents() const
{
	...
	LOG_(ImageLog,plog::debug) << "Image::getComponents \t" << components << "\n";
    return components;
}
...
unsigned char* Image::getPixels()
{
	...
	LOG_(ImageLog,plog::debug) << "Image::getpixels \n";
    return pixels;
}
...
unsigned char* Image::getPixelRow(int mip, int row)
{
  clog << "Image::getPixelRow \t" << mip << "\t" << row << "\n";
	LOG_(ImageLog,plog::debug) << "Image::getPixelRow \t" << mip << "\t" << row << "\n";
...
unsigned char* Image::getPixelRow(int row)
{
	...
	LOG_(ImageLog,plog::debug)  << "Image::getPixelRow \t" << row << "\n";
...
unsigned char* Image::getMipLevel(int mip)
{
	...
	LOG_(ImageLog,plog::debug) << "Image::getMipLevel \t" << mip << "\n";
    if (mip >= mipLevels)
        return NULL;
...
int Image::getMipLevelSize(int mip) const
{
	...
	LOG_(ImageLog,plog::debug)  << "Image::getMipLevelSize \t" << mip << "\n";
    if (mip >= mipLevels)
        return 0;
    else
...
bool Image::isCompressed() const
{
	...
	LOG_(ImageLog,plog::debug) << "Image::isCompressed \t" << format << "\n";
    switch (format)
    {
    case GL_COMPRESSED_RGBA_S3TC_DXT1_EXT:
    case GL_COMPRESSED_RGBA_S3TC_DXT3_EXT:
...
bool Image::hasAlpha() const
{
	...
	LOG_(ImageLog,plog::debug) << "Image::hasAlpha \t" << format << "\n";
    switch (format)
    {
    case GL_COMPRESSED_RGBA_S3TC_DXT3_EXT:
    case GL_COMPRESSED_RGBA_S3TC_DXT5_EXT:
...
Image* Image::computeNormalMap(float scale, bool wrap) const
{
  clog << "Image::computeNormalMap \t" << scale << "\t" << wrap << "\n";
	LOG_(ImageLog,plog::debug)  << "Image::computeNormalMap \t" << scale << "\t" << wrap << "\n";
...
Image* LoadImageFromFile(const string& filename)
{
	...
	LOG_(ImageLog,plog::debug)  << _("Loading image from file ") << filename << '\n';
    switch (type)
    {
    case Content_JPEG:
    ...
    default:
		...
		LOG_(ImageLog,plog::error) << _(": unrecognized or unsupported image file type.\n");
        break;
    }
...
{
      clog << "my_error_exit \n";
	  LOG_(ImageLog,plog::error) << "Error exit";
...
Image* LoadJPEGImage(const string& filename, int)
{
  clog << "LoadJPEGImage \t" << filename << "\t" << "\n";
	LOG_(ImageLog,plog::debug)  << "LoadJPEGImage \t" << filename << "\t" << "\n";
#ifdef JPEG_SUPPORT
    Image* img = NULL;
